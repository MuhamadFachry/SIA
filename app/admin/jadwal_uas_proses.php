...
$query = mysqli_query($koneksi,"select * from jadwal_ujian where status='uas'");
...
if ($cek==0) {
	...
	if (move_uploaded_file($tmp_file, $path)) {
		$query=mysqli_query($koneksi,"insert into jadwal_ujian (file,status) values ('$nama_file','uas')");
		if ($query) {
	...
	echo ("<SCRIPT LANGUAGE='JavaScript'>
		    window.alert('Jadwal UAS sudah ada')
		    window.location.href='jadwal_uas.php';
		    </SCRIPT>");
